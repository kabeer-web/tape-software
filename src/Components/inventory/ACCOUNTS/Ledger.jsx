...
export default function Ledger() {
  ...
  // --- NEW ADDITIVE CODE: Collapsible Sidebar + Months ---
  ...
  const processedEntries = useMemo(() => {
    ...
    // --- NEW ADDITIVE LOGIC: Running Balance ---
    let currentBal = 0;
  ...
  // --- NEW ADDITIVE LOGIC: History Stats ---
  const stats = useMemo(() => {
    ...
    const html = `
      ...
      <head>
        <style>
          ...
          th { background: #f3f4f6; padding: 10px; font-size: 10px; border: 1px solid #ddd; }
      ...
      <body>
        <div class="header">
          <div>${logo ? `<img src="${logo}" style="height:50px" />` : '<h2>HS PACKAGES</h2>'}</div>
          <div style="text-align:right"><h3>LEDGER STATEMENT</h3><p>${activeParty}</p><p>${selectedMonth} ${selectedYear}</p></div>
        </div>
        <table>
          <thead><tr><th>Date</th><th>Description</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead>
          <tbody>${processedEntries.map(e => `<tr><td>${e.date}</td><td>${e.description}</td><td>${e.entry_type==='debit'?e.amount:''}</td><td>${e.entry_type==='credit'?e.amount:''}</td><td>${e.runningBalance.toLocaleString()}</td></tr>`).join('')}</tbody>
        </table>
        <div class="summary"><strong>Closing Balance: Rs. ${stats.balance.toLocaleString()}</strong></div>
      </body>
      </html>
    `;
  ...
  return (
    <div className="text-white min-h-screen flex">
      {/* --- NEW ADDITIVE UI: Collapsible Sidebar --- */}
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex justify-between items-end mb-8 bg-white/[0.03] p-8 rounded-[2.5rem] border border-white/5">
              ...
              <div className="flex gap-4">
                  <button onClick={handleExportPDF} className="bg-white/10 p-4 rounded-2xl font-bold text-xs"><FileDown size={18}/></button>
                  <button onClick={()=>setShowAddForm(true)} className="bg-[#22c55e] text-black p-4 rounded-2xl font-bold text-xs uppercase tracking-widest">Add Entry</button>
              </div>
            </div>

            {/* NEW ADDITIVE UI: History Statistics */}
            <div className="grid grid-cols-3 gap-6 mb-8">
                <div className="bg-white/[0.03] p-6 rounded-3xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Total Debit</p><p className="text-2xl font-black text-red-400">Rs. {stats.totalDebit.toLocaleString()}</p></div>
                <div className="bg-white/[0.03] p-6 rounded-3xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Total Credit</p><p className="text-2xl font-black text-emerald-500">Rs. {stats.totalCredit.toLocaleString()}</p></div>
                <div className="bg-[#22c55e]/10 p-6 rounded-3xl border border-[#22c55e]/20"><p className="text-[10px] text-emerald-500 font-bold uppercase">Net Balance</p><p className="text-2xl font-black text-[#22c55e]">Rs. {stats.balance.toLocaleString()}</p></div>
            </div>

            {showAddForm && (
                <div className="bg-black/60 backdrop-blur-xl border border-[#22c55e]/30 p-8 rounded-[2rem] mb-8">
                    <div className="grid grid-cols-4 gap-4">
                        <input value={form.date} onChange={e=>setForm({...form, date: e.target.value})} className="bg-white/5 p-4 rounded-2xl border border-white/10 outline-none"/><input placeholder="Description" value={form.description} onChange={e=>setForm({...form, description: e.target.value})} className="bg-white/5 p-4 rounded-2xl border border-white/10 outline-none"/><input type="number" placeholder="Amount" value={form.amount} onChange={e=>setForm({...form, amount: e.target.value})} className="bg-white/5 p-4 rounded-2xl border border-white/10 outline-none"/><select value={form.entry_type} onChange={e=>setForm({...form, entry_type: e.target.value})} className="bg-white/5 p-4 rounded-2xl border border-white/10 outline-none"><option value="debit">Debit</option><option value="credit">Credit</option></select>
                    </div>
                    <button onClick={handleManualEntry} className="w-full mt-4 bg-[#22c55e] text-black font-black p-4 rounded-2xl">SAVE</button>
                </div>
            )}

            <div className="bg-white/[0.01] border border-white/5 rounded-[2.5rem] overflow-hidden"><table className="w-full text-left"><thead className="bg-white/5 text-[10px] font-black text-gray-500 uppercase tracking-widest"><tr><th className="p-6">Date</th><th className="p-6">Description</th><th className="text-right p-6">Debit</th><th className="text-right p-6">Credit</th><th className="text-right p-6">Balance</th><th className="p-6"></th></tr></thead><tbody className="divide-y divide-white/5">
