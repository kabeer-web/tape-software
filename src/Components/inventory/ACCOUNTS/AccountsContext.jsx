...
export const AccountsProvider = ({ children }) => {
  ...
  // Balance sign matches Ledger.jsx's balanceForType: Sale = debit − credit,
  // Purchase = credit − debit (this shop's convention — see the note above
  // handlePostLedger, and Ledger.jsx).
  const partiesSummary = useMemo(() => {
    ...
    const process = (name, type, amount, isDebit) => {
      ...
      summary[n].balance = summary[n].type === 'Purchase'
        ? summary[n].credit - summary[n].debit
        : summary[n].debit - summary[n].credit;
    };
  ...
  // stays correct): entry_type 'credit' = Sale bill (customer side).
  // entry_type 'debit' = Purchase bill (supplier side). This is a
  // shop-specific bookkeeping choice, not universal double-entry
  // terminology — what matters is that Sale and Purchase always post
  // opposite entry_types, so balance = debit − credit stays meaningful.
