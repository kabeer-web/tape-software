    ...
    <div className="text-white min-h-screen">
      ...
      <div className="bg-white/[0.03] rounded-2xl border border-[#22c55e]/10 overflow-x-auto">
        <table className="w-full text-left min-w-[760px]">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} className="p-10 text-center text-gray-500">Koi roll nahi mila.</td></tr>
            ) : filtered.map(item => {
              ...
              return (
                <tr key={item._id} className={`border-t border-white/5 hover:bg-white/[0.02] ${isNegative ? 'bg-red-500/5' : isLow ? 'bg-yellow-500/5' : ''}`}>
                  ...
                  <td className="p-3 text-sm text-gray-400">
                    {isEditing ? (
                      <select value={editForm.color} onChange={e => setEditForm(f => ({ ...f, color: e.target.value }))} className="bg-black/30 p-1.5 rounded-lg border border-[#22c55e]/40 outline-none text-sm">
                        {COLORS.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    ) : item.color}
                  </td>
