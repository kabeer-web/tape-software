...
const Tan = () => { // Capitalized component name (Standard Practice)
  ...
  // FIX: 'Tan' (Capital T) use karein taake StockContext ki JAMBO_CATS list se match ho
  const filtered = inventory.filter(i =>
    (i.category === 'Tan' || i.type === 'Tan') &&
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-[#22c55e] font-bold animate-pulse">Loading Tan Stock...</div>;

  return (
    <div className="text-white min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-black text-[#22c55e]">TAN TAPE</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white/[0.03] p-5 rounded-2xl border border-[#22c55e]/20">
          ...
          <div className="relative">
            ...
            <input placeholder="Roll number likhein..." className="w-full pl-10 p-3 bg-black/40 rounded-xl border border-[#22c55e]/20 outline-none focus:border-[#22c55e] transition-all text-sm text-white placeholder:text-gray-500" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={item._id} className={`border-t border-white/5 hover:bg-white/[0.02] transition-colors ${isLow ? 'bg-red-500/5' : ''}`}>
                  <td className="p-4 font-black text-[#22c55e] tracking-tighter">#{displayRoll(item.rollNo || item.roll_no)}</td>
                  <td className="p-4">
