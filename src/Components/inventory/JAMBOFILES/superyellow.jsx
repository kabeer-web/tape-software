...
const SuperYellow = () => {
  ...
  const filtered = inventory.filter(i =>
    (i.category === 'SuperYellow' || i.type === 'SuperYellow') &&
 ...
 const handleAdd = async (e) => {
  ...
  return (
    <div className="text-white min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-black text-[#22c55e]">SUPER YELLOW TAPE</h1>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={item._id} className={`border-t border-white/5 hover:bg-white/[0.02] ${isLow ? 'bg-yellow-500/5' : ''}`}>
                  <td className="p-3 font-bold text-sm">{displayRoll(item.rollNo || item.roll_no)}</td>
                  <td className="p-3 text-sm text-gray-400">{item.micron}μ / {item.width}mm</td>
                  <td className="p-3">
                    {isEditing ? (
