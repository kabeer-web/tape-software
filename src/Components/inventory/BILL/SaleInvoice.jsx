...
const SelectOrCustom = ({ value, onChange, options, placeholder }) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {custom && <input autoFocus value={value} onChange={e => onChange(e.target.value)} placeholder="Type value..." className="bg-black/40 p-2 rounded-xl border border-emerald-500/50 outline-none text-sm text-white" />}
    </div>
  );
};
...
const SaleInvoice = () => {
  ...
  const handleRemoveCarton = async () => {
    if (!carton.brand || !carton.qty || carton.qty <= 0) { alert("Brand aur Qty check karein"); return; }
    const match = inventory.find(i => 
      String(i.brand).toLowerCase() === String(carton.brand).toLowerCase() && 
      (String(i.category).toLowerCase() === 'carton' || String(i.type).toLowerCase() === 'carton') && 
      String(i.carton_type || i.cartonType || '').toLowerCase() === String(carton.type).toLowerCase() && 
    ...
    if (match) {
      ...
      setPopMsg(`✅ REMOVED: ${carton.qty} Cartons removed from ${carton.brand} stock.`);
      setTimeout(() => setPopMsg(''), 5000);
      refreshInventory();
    } else { alert("Matching carton inventory mein nahi mila."); }
  };
  ...
  const handleSave = async () => {
    ...
    setPopMsg(`✅ Bill Saved Successfully!`);
  ...
  return (
    <div className="text-white min-h-screen pb-10 max-w-6xl mx-auto px-4">
      ...
      <div className="flex justify-between items-center mb-8 pt-8">
        <div><h1 className="text-4xl font-black italic text-emerald-500">HS <span className="text-white">PACKAGES</span></h1></div>
        <div className="flex gap-3">
          ...
          <button onClick={() => {const h = generateInvoiceHTML({billNo, partyName:buyerName, date, items:rows, grandTotal: rows.reduce((s,r)=>s+r.total,0), totalCartonCount: rows.reduce((s,r)=>s+Number(r.totalCarton),0), logo}); const w = window.open('','_blank'); w.document.write(h); w.document.close();}} className="bg-emerald-500 text-black px-6 py-3 rounded-2xl font-black text-xs">PRINT</button>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2.5rem] mb-6 flex flex-col md:flex-row gap-8 items-center">
        <div className="shrink-0">
          {logo ? (
            <div className="relative group"><img src={logo} className="h-20 w-auto object-contain"/><button onClick={()=>setLogo(null)} className="absolute -top-2 -right-2 bg-red-600 rounded-full p-1"><X size={12}/></button></div>
          ) : (
            <button onClick={()=>fileRef.current?.click()} className="w-24 h-20 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-gray-500 hover:text-emerald-500 transition-all"><Upload size={24}/><span className="text-[8px] font-black mt-1">LOGO</span></button>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 w-full text-white">
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Bill No</label><input value={billNo} onChange={e=>setBillNo(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500 font-bold"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Buyer</label><input value={buyerName} onChange={e=>setBuyerName(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500 font-bold"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Date</label><input value={date} onChange={e=>setDate(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none font-bold"/></div>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2.5rem] mb-6">
         <div className="flex bg-black/40 p-1 rounded-2xl border border-white/5 w-fit mb-6">
            <button onClick={() => upd('sizeUnit', 'mm')} className={`px-8 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${form.sizeUnit === 'mm' ? 'bg-emerald-500 text-black' : 'text-gray-500'}`}>MM</button>
            <button onClick={() => upd('sizeUnit', 'inch')} className={`px-8 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${form.sizeUnit === 'inch' ? 'bg-emerald-500 text-black' : 'text-gray-500'}`}>Inches</button>
         </div>
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {form.sizeUnit === 'mm' ? <SelectOrCustom value={form.sizeMm} onChange={v=>upd('sizeMm',v)} options={['720','900','1280','1600']} placeholder="Size MM"/> : <SelectOrCustom value={form.sizeInch} onChange={v=>upd('sizeInch',v)} options={SIZE_INCH} placeholder="Size INCH"/>}
            <SelectOrCustom value={form.yards} onChange={v=>upd('yards',v)} options={['40','50','80','100','150','200']} placeholder="Yards"/>
            <SelectOrCustom value={form.colour} onChange={v=>upd('colour',v)} options={['Clear','Tan','Cloth','Masking','Tissue','Foam']} placeholder="Colour"/>
            <SelectOrCustom value={form.brand} onChange={v=>upd('brand',v)} options={['Bell','Race','Tesco','Jhonson']} placeholder="Brand"/>
            ...
            <button onClick={addItem} className="col-span-2 bg-emerald-500 text-black font-black p-4 rounded-2xl hover:scale-[1.02] transition-all">ADD TO BILL</button>
         </div>
      </div>
      ...
      <div className="bg-amber-500/5 border border-amber-500/20 p-8 rounded-[2.5rem] mb-8 relative">
        <div className="flex items-center justify-between mb-6">
           <div className="flex items-center gap-3"><Box className="text-amber-500" size={24}/><h3 className="text-white font-bold">MANUAL STOCK DEDUCT (CARTONS)</h3></div>
           {availableStock !== null && <div className="px-4 py-2 bg-black/40 rounded-xl font-black text-xs border border-amber-500/30 text-amber-400">Available: {availableStock} CTN</div>}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <button onClick={handleRemoveCarton} className="bg-amber-500 text-black font-black p-4 rounded-2xl hover:scale-105 active:scale-95 transition-all text-xs">DEDUCT STOCK</button>
        </div>
      </div>
      ...
      <div className="bg-white/[0.01] border border-white/5 rounded-[2.5rem] overflow-hidden mb-8 shadow-inner">
        <table className="w-full text-left">
          <thead className="bg-white/5 text-[10px] font-black text-gray-500 uppercase tracking-widest"><tr><th className="p-6">Description</th><th className="text-center">CTN</th><th className="text-center">Total</th><th className="text-right p-6">Amount</th><th className="p-6"></th></tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-white/5 transition-all"><td className="p-6 font-bold text-white leading-tight uppercase tracking-tighter">{r.brand} - {r.colour}<p className="text-[10px] text-gray-500 mt-1 uppercase italic">{r.sizeLabel}</p></td><td className="text-center font-black text-emerald-500 text-2xl">{Number(r.totalCarton)}</td><td className="text-center text-gray-400 font-bold">{Number(r.totalQty)} <small className="text-[8px] opacity-40">PCS</small></td><td className="text-right font-black text-white text-2xl p-6">Rs. {r.total.toLocaleString()}</td><td className="p-6 text-right"><button onClick={()=>setRows(rows.filter(x=>x.id!==r.id))} className="bg-red-500/10 p-3 rounded-xl text-red-500 hover:bg-red-500 transition-all"><Trash2 size={16}/></button></td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col justify-center shadow-lg"><p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3 italic">Amount In Words</p><p className="text-sm font-black text-slate-400 italic">"${toWords(rows.reduce((s,r)=>s+r.total,0))}"</p></div>
        <div className="bg-emerald-500 p-8 rounded-[3rem] flex items-center justify-between shadow-2xl text-emerald-950 font-black uppercase"><div className="tracking-tighter leading-tight"><span className="opacity-60 text-xs">Grand Total Dispatch</span><p className="text-lg">{rows.reduce((s,r)=>s+Number(r.totalCarton),0)} Units</p></div><p className="text-5xl tracking-tighter">Rs. {rows.reduce((s,r)=>s+r.total,0).toLocaleString()}</p></div>
      </div>
    </div>
  );
};
