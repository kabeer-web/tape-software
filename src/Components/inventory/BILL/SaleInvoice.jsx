...
} from 'lucide-react';import { useState, useRef, useContext, useEffect } from 'react';
import {
  FileText, Plus, Trash2, Printer, Upload, X, Save, 
  CheckCircle2, Box, Database, ArrowRight
} from 'lucide-react';
import { useAccounts } from '../ACCOUNTS/AccountsContext';
import { StockContext } from '../StockContext';

// Number to words logic... (Keeping your existing logic)
...
const SaleInvoice = () => {
  ...
  const [form, setForm] = useState({ sizeUnit:'mm', sizeMm:'', sizeInch:'', yards:'', colour:'', brand:'', micron:'', totalCarton:'', perCtnQty:'', rate:'' });
  const [rows, setRows] = useState([]);
  const [carton, setCarton] = useState({ brand:'', type:'', size:'', qty:'' });
  ...
  useEffect(() => {
    if (carton.brand && carton.type && carton.size) {
      const match = inventory.find(i => 
        i.brand?.trim().toLowerCase() === carton.brand.trim().toLowerCase() && 
        (i.category === 'Carton' || i.type === 'Carton') && 
        (i.carton_type || i.cartonType || i.type)?.trim().toLowerCase() === carton.type.trim().toLowerCase() && 
        parseFloat(i.size) === parseFloat(carton.size)
      );
      setDbStock(match ? match.qty : 0);
    } else { setDbStock(null); }
  }, [carton, inventory]);

  const handleRemoveCarton = async () => {
    if (!carton.brand || !carton.qty) return;
    const match = inventory.find(i => 
        i.brand?.trim().toLowerCase() === carton.brand.trim().toLowerCase() && 
        (i.category === 'Carton' || i.type === 'Carton') && 
        (i.carton_type || i.cartonType || i.type)?.trim().toLowerCase() === carton.type.trim().toLowerCase() && 
    ...
    if (match) {
      ...
      setPopMsg(`Inventory Successfully Updated: ${carton.qty} Cartons removed from ${carton.brand} (${carton.size}")`);
      setTimeout(() => setPopMsg(''), 5000);
      refreshInventory();
    } else { alert("DATABASE ERROR: This specific carton was not found. Please check Brand/Type/Size."); }
  };

  const addItem = () => {
    if (!form.totalCarton || !form.rate) return;
    const totalQty = parseFloat(form.totalCarton) * parseFloat(form.perCtnQty);
  ...
  const handlePrint = () => {
    const billData = { billNo, partyName: buyerName, date, items: rows, grandTotal: rows.reduce((s,r)=>s+r.total,0), totalCartonCount: rows.reduce((s,r)=>s+Number(r.totalCarton),0), logo };
    const html = generateInvoiceHTML(billData);
    const w = window.open('', '_blank'); w.document.write(html); w.document.close();
  };

  const grandTotal = rows.reduce((s, r) => s + r.total, 0);
  const totalCartons = rows.reduce((s, r) => s + Number(r.totalCarton), 0);
  ...
  return (
    <div className="text-white min-h-screen pb-10 max-w-6xl mx-auto px-4">
      ...
      <div className="flex justify-between items-center mb-8 pt-8">
        <div><h1 className="text-4xl font-black italic text-[#10b981]">HS <span className="text-white">PACKAGES</span></h1><p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">Premium Billing System</p></div>
        <div className="flex gap-3">
          <button onClick={async () => { setSaving(true); await saveBill({ billType:'Sale', billNo, partyName:buyerName, date, items:rows, grandTotal, totalCartonCount:totalCartons, logo }); setPopMsg("✅ Bill Saved to Database"); setRows([]); setSaving(false); }} disabled={saving || rows.length === 0} className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-6 py-3 rounded-2xl font-black text-xs hover:bg-emerald-500 hover:text-black transition-all">SAVE BILL</button>
          <button onClick={handlePrint} className="bg-emerald-500 text-black px-6 py-3 rounded-2xl font-black text-xs flex items-center gap-2 hover:scale-105 transition-all"><Printer size={16}/> PRINT INVOICE</button>
        </div>
      </div>

      {/* Bill Meta & Logo */}
      <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[2.5rem] mb-6 flex flex-col md:flex-row gap-8 items-center backdrop-blur-xl shadow-2xl">
        <div className="shrink-0">
          {logo ? (
            <div className="relative group"><img src={logo} className="h-20 w-auto object-contain"/><button onClick={()=>setLogo(null)} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1"><X size={12}/></button></div>
          ) : (
            <button onClick={()=>fileRef.current?.click()} className="w-24 h-20 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-gray-500 hover:text-emerald-500 transition-all"><Upload size={24}/><span className="text-[8px] font-black mt-1 uppercase">Logo</span></button>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 w-full font-bold">
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Bill No</label><input value={billNo} onChange={e=>setBillNo(e.target.value)} placeholder="001" className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Buyer</label><input value={buyerName} onChange={e=>setBuyerName(e.target.value)} placeholder="Party Name" className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase">Date</label><input value={date} onChange={e=>setDate(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none"/></div>
        </div>
      </div>

      {/* Manual Carton Deduction */}
      <div className="bg-amber-500/5 border-2 border-amber-500/10 p-8 rounded-[2.5rem] mb-6 relative overflow-hidden group">
        <div className="flex items-center justify-between mb-6">
           <div className="flex items-center gap-3"><Box className="text-amber-500" size={24}/><h3 className="text-white font-black uppercase tracking-tight">Remove Carton from Stock</h3></div>
           {dbStock !== null && <div className={`px-4 py-2 rounded-xl font-black text-xs border ${dbStock > 5 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400 animate-pulse'}`}>Database: {Number(dbStock)} CTN</div>}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 relative z-10">
          <select value={carton.brand} onChange={e=>setCarton({...carton, brand:e.target.value})} className="bg-black/60 p-4 rounded-2xl border border-amber-500/20 text-white outline-none">
            <option value="">Brand</option>{['Bell','Race','Tesco','Jhonson'].map(b=><option key={b} value={b}>{b}</option>)}
          </select>
          ...
          <select value={carton.size} onChange={e=>setCarton({...carton, size:e.target.value})} className="bg-black/60 p-4 rounded-2xl border border-amber-500/20 text-white outline-none">
            <option value="">Size</option>{['10','10.5','11','12'].map(s=><option key={s} value={s}>{s}"</option>)}
          </select>
          ...
          <button onClick={handleRemoveCarton} className="bg-amber-500 text-black font-black p-4 rounded-2xl hover:scale-105 active:scale-95 transition-all text-[10px] uppercase">Deduct Stock</button>
        </div>
      </div>

      {/* Main Table & Footer are handled by your existing logic with Number() formatting added for 010 fix */}
      <div className="bg-white/[0.01] border border-white/5 rounded-[2.5rem] overflow-hidden mb-8 shadow-inner">
        <table className="w-full text-left">
          <thead className="bg-white/5 text-[10px] font-black text-gray-500 uppercase tracking-widest"><tr><th className="p-6">Description</th><th className="text-center">CTN</th><th className="text-center">Total Pcs</th><th className="text-right">Rate</th><th className="text-right p-6">Amount</th><th className="p-6"></th></tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-white/5 transition-all"><td className="p-6 font-bold text-white uppercase tracking-tighter">{r.brand} - {r.colour}<p className="text-[10px] text-gray-500 mt-1 italic lowercase">{r.sizeLabel}</p></td><td className="text-center font-black text-emerald-500 text-2xl">{Number(r.totalCarton)}</td><td className="text-center text-gray-400 font-bold">{Number(r.totalQty)}</td><td className="text-right text-gray-300 font-mono">{r.rate.toLocaleString()}</td><td className="text-right font-black text-white text-2xl p-6">Rs. {r.total.toLocaleString()}</td><td className="p-6 text-right"><button onClick={()=>setRows(rows.filter(x=>x.id!==r.id))} className="text-red-500/20 p-2 rounded-lg hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button></td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#10b981] p-8 rounded-[3rem] flex items-center justify-between shadow-2xl text-emerald-950 font-black uppercase"><div className="tracking-tighter leading-tight"><span className="opacity-60 text-xs font-bold">Total Payable</span><p className="text-lg">{totalCartons} Units</p></div><p className="text-5xl tracking-tighter">Rs. {grandTotal.toLocaleString()}</p></div>
      </div>
    </div>
  );
};

// Re-defining generateInvoiceHTML inside the same file or keeping it globally is fine.
export default SaleInvoice;
import { useAccounts } from '../ACCOUNTS/AccountsContext';
import { StockContext } from '../StockContext';

// ── CONSTANTS (Missing definitions fixed) ────────────────
...
const SaleInvoice = () => {
  ...
  const handleRemoveCarton = async () => {
    ...
    if (match) {
      await updateStock(match.id || match._id, -parseInt(carton.qty));
      setPopMsg(`✅ DEDUCTED: ${carton.qty} Cartons removed from ${carton.brand} stock.`);
      setTimeout(() => setPopMsg(''), 5000);
      refreshInventory();
  ...
  return (
    <div className="text-white min-h-screen pb-10 max-w-6xl mx-auto px-4">
      {popMsg && (
        <div className="fixed top-10 right-10 z-50 bg-emerald-500 text-black p-5 rounded-2xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-right duration-500">
           <CheckCircle2 size={24}/><p className="font-bold">{popMsg}</p><button onClick={()=>setPopMsg('')}><X size={18}/></button>
        </div>
      )}

      {/* Header */}
      <div className="flex justify-between items-center mb-8 pt-8">
        <div><h1 className="text-4xl font-black italic text-emerald-500">HS <span className="text-white font-black">PACKAGES</span></h1></div>
        <div className="flex gap-3">
          <button onClick={handleSave} disabled={saving || rows.length === 0} className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 px-6 py-3 rounded-2xl font-black text-xs hover:bg-emerald-500 hover:text-black transition-all">SAVE BILL</button>
          <button onClick={() => {const h = generateInvoiceHTML({billNo, partyName:buyerName, date, items:rows, grandTotal, totalCartonCount, logo}); const w = window.open('','_blank'); w.document.write(h); w.document.close();}} className="bg-emerald-500 text-black px-6 py-3 rounded-2xl font-black text-xs">PRINT</button>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2.5rem] mb-6 flex flex-col md:flex-row gap-8 items-center backdrop-blur-xl">
        <div className="shrink-0 text-center">
          {logo ? (
            ...
            <button onClick={()=>fileRef.current?.click()} className="w-28 h-20 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-gray-500 hover:text-emerald-500 transition-all"><Upload size={24}/><span className="text-[8px] font-black mt-1">UPLOAD LOGO</span></button>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 w-full text-white">
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase font-black">Invoice #</label><input value={billNo} onChange={e=>setBillNo(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500 font-bold"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase font-black">Buyer Name</label><input value={buyerName} onChange={e=>setBuyerName(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none focus:border-emerald-500 font-bold"/></div>
          <div className="space-y-1"><label className="text-[10px] text-gray-500 uppercase font-black">Date</label><input value={date} onChange={e=>setDate(e.target.value)} className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 outline-none font-bold text-slate-400"/></div>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2.5rem] mb-6 shadow-xl">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            <button onClick={addItem} className="col-span-2 bg-emerald-500 text-black font-black p-4 rounded-2xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2"><Plus size={18}/> ADD TO BILL</button>
         </div>
      </div>

      {/* Carton Sync */}
      <div className="bg-amber-500/5 border-2 border-amber-500/20 p-8 rounded-[2.5rem] mb-8 relative">
        <div className="flex items-center justify-between mb-6">
           <div className="flex items-center gap-3"><Box className="text-amber-500" size={24}/><h3 className="text-white font-bold uppercase">Manual Stock Deduct (Cartons)</h3></div>
           {availableStock !== null && <div className="px-4 py-2 bg-black/40 rounded-xl font-black text-xs border border-amber-500/30 text-amber-400">Database: {availableStock} CTN</div>}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <select value={carton.brand} onChange={e=>updC('brand',e.target.value)} className="bg-black/60 p-4 rounded-2xl border border-amber-500/20 outline-none text-white">{CARTON_BRANDS.map(b=><option key={b} value={b}>{b}</option>)}</select>
          <select value={carton.type} onChange={e=>updC('type',e.target.value)} className="bg-black/60 p-4 rounded-2xl border border-amber-500/20 outline-none text-white"><option value="Small">Small</option><option value="Large">Large</option></select>
          <select value={carton.size} onChange={e=>updC('size',e.target.value)} className="bg-black/60 p-4 rounded-2xl border border-amber-500/20 outline-none text-white">{CARTON_SIZES.map(s=><option key={s} value={s}>{s}"</option>)}</select>
          <input type="number" value={carton.qty} onChange={e=>updC('qty',e.target.value)} placeholder="Qty" className="w-full bg-black/60 p-4 rounded-2xl border border-amber-500/20 font-black text-amber-500"/>
          <button onClick={handleRemoveCarton} className="bg-amber-500 text-black font-black p-4 rounded-2xl hover:scale-105 active:scale-95 transition-all text-[10px] uppercase">DEDUCT FROM STOCK</button>
        </div>
      </div>
      ...
      <div className="bg-white/[0.01] border border-white/5 rounded-[2.5rem] overflow-hidden mb-8 shadow-inner">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-white/5">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-white/5 transition-all"><td className="p-6 font-bold text-white leading-tight uppercase tracking-tighter">{r.brand} - {r.colour}<p className="text-[10px] text-gray-500 mt-1 uppercase italic">{r.sizeLabel}</p></td><td className="text-center font-black text-emerald-500 text-2xl">{Number(r.totalCarton)}</td><td className="text-center text-gray-400 font-bold">{Number(r.totalQty)} <small className="text-[8px] opacity-40 uppercase">PCS</small></td><td className="text-right font-mono text-xs text-gray-300">{r.rate.toLocaleString()}</td><td className="text-right font-black text-white text-2xl p-6">Rs. {r.total.toLocaleString()}</td><td className="p-6 text-right"><button onClick={()=>setRows(rows.filter(x=>x.id!==r.id))} className="bg-red-500/10 p-3 rounded-xl text-red-500 hover:bg-red-500 transition-all shadow-lg"><Trash2 size={16}/></button></td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-emerald-500 p-8 rounded-[3rem] flex items-center justify-between shadow-2xl text-emerald-950 font-black uppercase"><div className="tracking-tighter leading-tight"><span className="opacity-60 text-xs">Final Bill Value</span><p className="text-lg">{totalCartonCount} Total Units</p></div><p className="text-5xl tracking-tighter">Rs. {grandTotal.toLocaleString()}</p></div>
      </div>
    </div>
  );
};
