...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-[#0c0c0c] border-r border-[#22c55e]/10 flex flex-col transition-transform duration-300 ease-in-out transform
      ...
      `}>
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          ...
          <button onClick={onClose} className="lg:hidden p-2 text-gray-500 hover:text-white rounded-lg transition">
            <X size={20} />
          </button>
        </div>

        <div className="px-4 pb-2 shrink-0">
          <NavLink to="/search" onClick={handleNavClick} className={navLinkClass}>
        ...
        <div className="px-4 pb-5 pt-3 border-t border-[#22c55e]/10 shrink-0">
          <div className="flex items-center gap-3 px-3 py-2.5 mb-2 bg-white/[0.03] rounded-xl border border-white/5">
            <div className="w-8 h-8 rounded-xl bg-[#22c55e]/20 border border-[#22c55e]/30 flex items-center justify-center shrink-0 text-[#22c55e] font-black text-sm uppercase">
              {user?.email?.[0] || 'U'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs text-white font-semibold truncate">{user?.email || 'User'}</p>
              <p className="text-[10px] text-[#22c55e]">Admin</p>
            </div>
          </div>
          <button
            onClick={() => window.confirm('Logout?') && signOut()}
            className="flex items-center gap-3 px-4 py-2.5 w-full text-sm text-red-400 hover:text-red-300 hover:bg-red-500/5 rounded-xl transition"
          >
...
export default Sidebar; // YE LINE ZAROORI HAI
